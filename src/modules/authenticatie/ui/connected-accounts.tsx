...
export function ConnectedAccounts() {
	...
	const [providerToUnlink, setProviderToUnlink] = useState<TOAuthProvider | null>(null);
	const [unlinkConfirmationText, setUnlinkConfirmationText] = useState('');
	const UNLINK_CONFIRMATION_PHRASE = "unlink account";
	const isUnlinkButtonDisabled = unlinkConfirmationText !== UNLINK_CONFIRMATION_PHRASE;
	...
	async function confirmUnlink() {
		...
		try {
			const result = await unlinkOAuthAccount(providerToUnlink);

			if (result.success) {
				setAccounts(accounts.filter((account) => account.provider !== providerToUnlink));
				toast.success('Account unlinked successfully');
				setShowUnlinkConfirmation(false);
				setUnlinkConfirmationText('');
	...
	async function handleLink(provider: TOAuthProvider) {
		try {
			...
			if (provider === 'google') {
				const url = await generateGoogleAuthUrl('/space');
				window.location.href = url;
			} else if (provider === 'github') {
				const url = await generateGitHubAuthUrl('/space');
				window.location.href = url;
			} else if (provider === 'discord') {
				const url = await generateDiscordAuthUrl('/space');
				window.location.href = url;
			} else {
	...
	return (
		<>
			<Card className="border border-border/40 shadow-sm scale-in">
				<CardHeader>
					<CardTitle className="text-xl page-header">Connected Accounts</CardTitle>
					<CardDescription>
						Manage your connected services and authentication methods
					</CardDescription>
				</CardHeader>
				<CardContent>
					<div className="space-y-4">
						...
						{accounts.map((account) => (
							<div
								key={account.id}
								className="group flex items-center justify-between rounded-lg border border-border/40 p-4 transition-all hover:border-primary/30 hover:bg-accent/20"
							>
						...
						{unconnectedProviders.map((provider) => (
							<div
								key={provider}
								className="group flex items-center justify-between rounded-lg border border-dashed border-border p-4 bg-background/50 hover:border-primary/30 hover:bg-accent/10 transition-all"
							>
								<div className="flex items-center gap-4">
			...
			<Dialog 
				...
				onOpenChange={(open) => {
					setShowUnlinkConfirmation(open);
					if (!open) setUnlinkConfirmationText('');
				}}
			>
				<DialogContent className="sm:max-w-[425px]">
					<DialogHeader>
					...
					<div className="py-4 space-y-4">
						<Alert
						 variant="destructive">
							...
							<AlertDescription>
								Make sure you have another way to sign in to your account.
							</AlertDescription>
						</Alert>
						
						<div className="space-y-2">
							<Label htmlFor="confirm-unlink" className="text-sm font-medium">
								To confirm, type "{UNLINK_CONFIRMATION_PHRASE}" below:
							</Label>
							<Input
								id="confirm-unlink"
								value={unlinkConfirmationText}
								onChange={(e) => setUnlinkConfirmationText(e.target.value)}
								placeholder={UNLINK_CONFIRMATION_PHRASE}
							/>
						</div>
					</div>
					
					<DialogFooter>
						<Button variant="outline" onClick={() => setShowUnlinkConfirmation(false)}>
							Cancel
						</Button>
						<Button
							variant="default"
