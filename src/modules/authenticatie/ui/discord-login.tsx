...
export function DiscordLoginButton({ className, redirectTo = '/space' }: TProps) {
	const [isLoading, setIsLoading] = useState(false);

	const handleLogin = async () => {
		try {
			setIsLoading(true);
			const url = await generateDiscordAuthUrl(redirectTo);
	...
	return (
		...
		>
			{isLoading ? (
				...
				<DiscordLogo className="mr-2 h-4 w-4 translate-x-[3px]" />
			)}
		</Button>
	);
}
